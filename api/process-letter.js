...
1. 必ず「手紙をありがとう。」から始める。
2. 基本的には敬語を使うが、堅い表現は使わない。最後のメッセージはカジュアルに。ただし、元の手紙がカジュアルなら、口調を合わせてカジュアルにする。
...
10. 1つの段落に詰め込まず、話の内容がまとまりごとに変わるところで段落を分ける。段落と段落の間には、必ず空行（改行を2つ）を入れる。

返信は適度な長さ（300-400文字程度）で、2〜4段落程度に分けて書いてください。`,
        input: [
          {
            role: 'user',
            content: [
