...
1. 手紙の画像をアップロードする（または「写真でなく直接手紙を入力する場合はこちら」から入力する）
2. AI生成の返信を受け取る（不要なら「AI生成の返信を受け取らず、1年後への送信に進む」）
3. 1年後に送る（メールアドレスと送信日を入力して予約）

送信日は **1年後が初期値** で、前後の日付に変えられます。
選べるのは今日から2年後までです。**送信時刻は18時（日本時間）に固定**です。
